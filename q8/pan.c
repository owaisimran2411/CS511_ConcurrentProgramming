...
short src_ln3 [] = {
	  0,  69,  70,  71,  72,  75,  75,  77, 
	 77,  78,  78,  76,  80,  76,  75,  80, 
	 80,  80,  81,  80,  81,  81,  82,  81, 
	 83,  83,  83,  84,  83,  84,  74,  86, 
	  0, };
S_F_MAP src_file3 [] = {
	{ "-", 0, 0 },
	{ "tr.pml", 1, 31 },
...
short src_ln1 [] = {
	  0,  38,  39,  11,  12,  10,   9,  11, 
	 12,  10,   9,  15,  15,  11,  12,  10, 
	  9,  47,  15,  15,  15,  15,  51,  48, 
	 56,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "tr.pml", 1, 24 },
...
{	int j, h = now._nr_pr;
	...
	case 0:	/* PassengerTrain */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("PassengerTrain:i", ((P0 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			now.passengerCount[l_in] = 0;
		}
	}
		now.freightCount = 0;
#ifdef VAR_RANGES
		logval("permToLoad", now.permToLoad);
		logval("doneLoading", now.doneLoading);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("track[l_in]", now.track[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("passengerCount[l_in]", now.passengerCount[l_in]);
		}
	}
		logval("freightCount", now.freightCount);
#endif
}
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   permToLoad:	%d\n", now.permToLoad);
	printf("	byte   doneLoading:	%d\n", now.doneLoading);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   track[%d]:	%d\n", l_in, now.track[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   passengerCount[%d]:	%d\n", l_in, now.passengerCount[l_in]);
		}
	}
	printf("	byte   freightCount:	%d\n", now.freightCount);
}
void
...
{	/* int i; */
	...
	printf("	byte   i:	%d\n", ((P3 *)pptr(pid))->i);
		break;
	case 2:
		printf("local vars proc %d (LoadingMachine):\n", pid);
		break;
	case 1:
		/* none */
		break;
	case 0:
		printf("local vars proc %d (PassengerTrain):\n", pid);
	printf("	int    i:	%d\n", ((P0 *)pptr(pid))->i);
		break;
	}
}
